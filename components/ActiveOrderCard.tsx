...
import { MapPin, Navigation, MessageSquare, Camera, Info, Check, Zap } from 'lucide-react';
import { Order, OrderStatus } from '../types';

interface ActiveOrderCardProps {
  order: Order;
  onAction: (orderId: string, action: 'next_status' | 'chat' | 'map') => void;
}
...
export const ActiveOrderCard: React.FC<ActiveOrderCardProps> = ({ order, onAction }) => {
  ...
  return (
    <div className="bg-[#1a222e] rounded-3xl p-6 border border-white/5 shadow-2xl">
      {/* Header */}
      <div className="flex justify-between items-start mb-6">
        <div>
          <p className="text-yellow-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-1">
            PEDIDO {order.orderNumber}
          </p>
          <h3 className="text-white text-xl font-bold">{order.storeName || 'Market Central'}</h3>
        </div>
        <div className="text-right">
          <p className="text-gray-400 text-xs mb-1">Pago</p>
          <p className="text-[#4ade80] text-2xl font-bold">{formatCurrency(order.earnings)}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <button 
          onClick={() => onAction(order.id, 'next_status')}
        ...
        <div className="grid grid-cols-2 gap-3">
          <button 
            onClick={() => onAction(order.id, 'chat')}
